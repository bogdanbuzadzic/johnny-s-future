...
function MyMoneyContent() {
  ...
  const tips = ["You're spending less on food this week. Keep it up!", "Emergency fund is 40% there. Closer than you think!", "At this pace, vacation goal done by December.", "Try the What If button to explore scenarios."];
  return <SimulationProvider flexRemaining={flexRemaining} dailyAllowance={dailyAllowance} daysRemaining={daysRemaining} monthlyIncome={monthlyIncome} averageDailySpend={avgDailySpend}>
        ...
        <div className="px-4 pt-12 pb-4">
          ...
          <div className="flex items-center justify-between mb-5">
            ...
            <div className="glass-light rounded-full p-0.5 flex">
              <button onClick={() => setPeriod('month')} className={`px-3 py-1.5 rounded-full text-[12px] font-medium transition-colors ${period === 'month' ? 'bg-white/20 text-white' : 'text-white/50'}`}>
                Monthly
              </button>
              <button onClick={() => setPeriod('week')} className={`px-3 py-1.5 rounded-full text-[12px] font-medium transition-colors ${period === 'week' ? 'bg-white/20 text-white' : 'text-white/50'}`}>
                Weekly
              </button>
            </div>
      ...
      scale: 0.95
    }} onClick={() => setShowAddTx(true)} className="fixed bottom-20 right-4 z-30 w-14 h-14 rounded-full gradient-primary flex items-center justify-center shadow-lg" style={{
