...
export function TetrisContainer({
  ...
  const containerHeight = Math.max(typeof window !== 'undefined' ? window.innerHeight * 0.55 : 400, fixedBarHeight + savingsBarHeight + blockRowsHeight + 120);
  ...
  return <div>
      ...
      boxShadow: isOverBudget ? 'inset 0 0 20px rgba(255,255,255,0.03), 0 -8px 24px rgba(255,159,10,0.3)' : 'inset 0 0 20px rgba(255,255,255,0.03)',
      minHeight: containerHeight
        ...
        borderBottom: '1px solid rgba(255,255,255,0.08)'
          ...
          <div className="flex-1 truncate text-[11px] text-white/25">
            {fixedCategories.map((c, i) => <span key={c.id}>
                {i > 0 && ' · '}
        ...
        {/* Zone 2: Savings */}
        <div className="flex items-center px-3 gap-2 relative backdrop-blur-sm" style={{
        ...
        <div className="p-3">
          {/* Blocks */}
          {blockLayout.rows.map((row, ri) => <div key={ri} className="flex gap-1.5 mb-1.5">
              {row.map(block => {
            const spent = getCategorySpent(block.id, period);
            const txs = getCategoryTransactions(block.id);
            return <CategoryBlock key={block.id} id={block.id} name={block.name} icon={block.icon} tintColor={getTintColor(block.name)} budget={block.budget} spent={spent} width={block.computedWidth} height={block.computedHeight} transactions={txs} onUpdateBudget={(id, newBudget) => updateCategory(id, {
          ...
          {/* Empty Space */}
          {emptySpaceHeight > 40 && <div className="relative flex flex-col items-center justify-center" style={{
          height: Math.max(emptySpaceHeight, 80)
        }}>
              {/* Dotted grid */}
            ...
            opacity: 0.03
              ...
              {spaceRatio < 0.1 && spaceRatio > 0 && <div className="absolute bottom-16 flex items-end gap-0.5 z-10">
          ...
          {isOverBudget && <div className="flex items-center justify-center py-2">
